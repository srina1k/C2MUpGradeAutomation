...
public class XmlUtil {
    public static void copyTagValues(String sourceXml, String targetXml, String sourceTag, String targetTag) {
        ...
        try {
            ...
            Document sourceDoc =
                    builder.parse(new File(sourceXml));

            Document targetDoc =
                    builder.parse(new File(targetXml));

            NodeList sourceNodes =
                    sourceDoc.getElementsByTagName(sourceTag);

            NodeList targetNodes =
                    targetDoc.getElementsByTagName(targetTag);
            ...
            transformer.transform(
                    new DOMSource(targetDoc),
                    new StreamResult(new File(targetXml)));

        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    }
    public static void updateTagValue(
            String xmlPath,
            String tagName,
            String value) {
